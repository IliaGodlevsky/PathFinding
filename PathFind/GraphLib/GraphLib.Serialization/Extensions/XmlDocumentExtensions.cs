...
namespace GraphLib.Serialization.Extensions
{
    ...
    {
        public static XmlDocument ToXml(this XmlDocument document, GraphSerializationInfo info)
        {
            return document.WithChild(() => document.CreateElement(Graph).WithChildren(() => document.ToXmlNodes(info)));
        }

        private static IEnumerable<XmlNode> ToXmlNodes(this XmlDocument document, GraphSerializationInfo info)
        {
            yield return document.CreateElement(Dimensions).WithAttributes(info.DimensionsSizes);
            yield return document.ToXmlNode(info.VerticesInfo);
            yield return document.CreateElement(Range).WithAttributes(info.CostRange.ToArray());
        }

        private static XmlNode ToXmlNode(this XmlDocument document, VertexSerializationInfo[] infos)
        {
            return document.CreateElement(Vertices).WithChildren(() =>
            {
                return infos.Select(vertex => document.CreateElement(Vertex).WithChildren(() => document.ToXmlNodes(vertex)));
            });
        }

        private static IEnumerable<XmlNode> ToXmlNodes(this XmlDocument document, VertexSerializationInfo vertex)
        {
            yield return document.CreateElement(Obstacle).WithAttributes(vertex.IsObstacle);
            yield return document.CreateElement(Neighbours).WithChildren(() => document.ToXmlNodes(vertex.Neighbourhood));
        ...
        private static IEnumerable<XmlNode> ToXmlNodes(this XmlDocument document, INeighborhood neighborhood)
        {
